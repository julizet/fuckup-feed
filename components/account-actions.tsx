...
import { Trash2, RefreshCw, AlertTriangle, Mail, Loader2 } from 'lucide-react'
...
export function AccountActions({ userEmail }: AccountActionsProps) {
  ...
  async function handleResetAccount() {
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json()
        throw new Error(data.error || 'Failed to initiate account reset')
      }
      
      setSuccess('We sent a confirmation email to ' + userEmail + '. Click the link to delete all your posts and reset your account.')
      ...
      {!showDeleteConfirm && !showResetConfirm ? (
        <div className="flex flex-wrap gap-2">
          <Button
            ...
            className="gap-2 border-border hover:border-amber-500 hover:text-amber-500"
          >
        ...
        <div className="space-y-3">
          <div className="flex items-start gap-2 text-amber-500">
            <AlertTriangle className="h-5 w-5 flex-shrink-0 mt-0.5" />
            <div>
              <p className="text-sm font-semibold">Reset your account?</p>
              <p className="text-xs text-muted-foreground mt-1">
                This will delete all your posts and votes but keep your account. You will receive a confirmation email.
              </p>
            </div>
          </div>
          <div className="flex gap-2">
